 ...
 */
function wolkenpflug_preprocess_html(&$vars) {
$vars['rdf'] = new stdClass;
  ...
  if (theme_get_setting('wolkenpflug_custom_css_location') == TRUE) {
    $path =  theme_get_setting('wolkenpflug_custom_css_path');
    if (file_exists($path)) {
      drupal_add_css("$path",
        array(
          'group' => CSS_THEME,
  ...
  // Add general JS.
  drupal_add_js(drupal_get_path('theme', 'wolkenpflug') . '/js/scripts.js',
    array(
      'group' => JS_THEME,
      'preprocess' => TRUE,
      'weight' => '9999',
  ...
  if (theme_get_setting('wolkenpflug_tertiarymenu') == TRUE) {
  ...
  if (theme_get_setting('wolkenpflug_themelogo') == TRUE) {
 ...
 */
function wolkenpflug_html_head_alter(&$head_elements) {
 ...
 */
function wolkenpflug_preprocess_username(&$vars) {
 ...
 */
function wolkenpflug_breadcrumb($vars) {
 ...
 */
function wolkenpflug_preprocess_page(&$vars, $hook) {
 ...
 */
function wolkenpflug_menu_local_tasks(&$vars) {
  $output = '';
 ...
 */
function wolkenpflug_preprocess_node(&$vars) {
 ...
 */
function wolkenpflug_page_alter($page) {
  ...
  if (theme_get_setting('wolkenpflug_viewport') == FALSE) {
...
function wolkenpflug_image_style($variables) 
{
	if($variables['style_name'] == 'liste_900')
	{
